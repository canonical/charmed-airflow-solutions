resource "null_resource" "wait_for_postgresql" {
  ...
  provisioner "local-exec" {
    command = "juju wait-for model $(juju show-model ${var.model_uuid} --format=json | jq -r '.[keys[0]].name' | awk -F/ '{print $2}') --timeout=20m --query=\"apps['${var.postgresql.app_name}'].status == 'active'\""
  }
}
